<template>
  <div>
    <div>
      <q-radio v-model="selectedChangeType" val="all" id="changetype-all">
        全部
      </q-radio>
      <q-radio
      ...
      <q-radio v-model="selectedChangeType" val="address" id="changetype-address">
        地址
      </q-radio>
      <q-radio v-model="selectedChangeType" val="geocode" id="changetype-geocode">
        地理編碼
      </q-radio>
    </div>
    <q-input ref="pacInputEl" type="text" placeholder="Enter a location" />
    <q-select
      v-model="type"
      label="類型"
      :options="typeOptions"
    ...
    <div ref="infowindowEl">
      <div>
        <span ref="place-name" class="title"></span>
        <br />
        <span ref="place-address" id="place-address"></span>
      </div>
    </div>
...
const showPlaces = () => {
  clearPlaces()
  locations[type].forEach(putMarker(type))
  // locations.hotels.forEach(putMarker('hotels'))
  // locations.attractions.forEach(putMarker('attractions'))
}
